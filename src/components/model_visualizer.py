...
class ModelVisualizer:
    ...
    def plot_model_comparison(
        ...
        try:
            ...
            bars2 = ax.bar(x + width/2, df['CV Mean Accuracy'], width,
    ...
    def plot_best_model_performance(
        ...
        try:
            ...
            bars = ax1.bar(categories, scores, yerr=errors, capsize=10,
                          color=colors, alpha=0.8, edgecolor='black', linewidth=1.5)
            
            # Add value labels
            for bar, score in zip(bars, scores):
                height = bar.get_height()
                ax1.text(bar.get_x() + bar.get_width()/2., height + errors[bars.index(bar)],
                        f'{score:.4f}',
                        ha='center', va='bottom', fontsize=11, fontweight='bold')
            
            ax1.set_ylabel('Accuracy', fontsize=12, fontweight='bold')
            ax1.set_title(f'{model_name.replace("_", " ").title()}: Test vs CV Accuracy',
            ...
            ax2.scatter([1] * len(cv_scores), cv_scores, alpha=0.6, 
                       color='#2c3e50', s=50, zorder=3)
            ax2.axhline(y=test_score, color='#e74c3c', linestyle='--', 
                       linewidth=2, label=f'Test Accuracy: {test_score:.4f}')
            ...
            plt.suptitle(f'Best Model Performance: {model_name.replace("_", " ").title()}',
    ...
    def plot_confusion_matrix_heatmap(
        ...
        try:
            ...
            sns.heatmap(confusion_matrix, annot=True, fmt='d', cmap='Blues',
                       ax=ax, cbar_kws={'label': 'Count'}, linewidths=1, linecolor='black')
    ...
    def plot_metrics_comparison(
        ...
        try:
            ...
            bars = ax.barh(metric_labels, values, color=['#3498db', '#2ecc71', '#f39c12', 
                                                          '#e74c3c', '#9b59b6'], alpha=0.8)
            ...
            ax.set_title(f'Performance Metrics: {model_name.replace("_", " ").title()}',
                        fontsize=14, fontweight='bold', pad=15)
            ax.set_xlim([0, 1])
