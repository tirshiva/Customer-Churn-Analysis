    ...
    def preprocess(self, data: pd.DataFrame) -> Optional[pd.DataFrame]:
        ...
        try:
            ...
            if self.dimension_reducer is not None:
                ...
                X_features = (
                    processed_data.drop(self.target_column, axis=1)
                    if self.target_column in processed_data.columns
                    else processed_data
                )
                ...
                if X_reduced is not None:
                    ...
                    if self.target_column in processed_data.columns:
                        processed_data = pd.concat(
                            [X_reduced, processed_data[[self.target_column]]], axis=1
                        )
                    else:
            ...
            if self.target_column in processed_data.columns:
                self.feature_columns = [
                    col for col in processed_data.columns if col != self.target_column
                ]
            else:
